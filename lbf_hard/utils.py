...
class FixedEntityObservationWrapper:
    ...
    def __init__(self, env, *, max_agents: int, env_food: int, max_food_slots: int, sight: int):
        self.env = env
        self.possible_agents = list(env.possible_agents)
        self.max_agents = int(max_agents)
        self.env_food = int(env_food)
        self.max_food_slots = int(max_food_slots)
        ...
        self._observation_size = 1 + (3 * self.max_food_slots) + (3 * max(0, self.max_agents - 1))

        raw_space = env.observation_space(self.possible_agents[0])
        raw_high = np.asarray(raw_space.high, dtype=np.float32).reshape(-1)
        max_food_level = float(np.max(raw_high[2 : (3 * self.env_food) : 3]))
        player_start = 3 * self.env_food
    ...
    def _encode_observation(self, raw_obs: np.ndarray) -> np.ndarray:
        ...
        for slot_index in range(self.env_food):
        ...
        player_section = flat[3 * self.env_food :]
...
def build_summary(
    *,
    ...
    selected_test_summary = final_test_summary
