...
if [ "${1:-}" = "" ]; then
    echo "Usage: bash server_scripts/run_rware_v6_ablations_cpu.sh RUN_NAME [MAX_PARALLEL]" >&2
    exit 1
fi
...
dst = results_root / "_task_configs" / "robotic_warehouse_dynamic_final_ablation.json"
...
discover_configs() {
    find "robotic_warehouse_dynamic/configs/pimac_v6_ablation" -maxdepth 1 -type f -name '*.json' | sort
}
...
launch_job() {
    ...
    local run_id="final_robotic_warehouse_dynamic_pimac_v6_ablation_${config_name}_s${seed}"
...
TASK_CONFIG_PATH="${RESULTS_ROOT}/_task_configs/robotic_warehouse_dynamic_final_ablation.json"
declare -a FAILED=()
running_jobs=0
...
if [ "${#configs[@]}" -eq 0 ]; then
    echo "No configs found under robotic_warehouse_dynamic/configs/pimac_v6_ablation" >&2
    exit 1
fi
