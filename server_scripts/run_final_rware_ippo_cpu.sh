...
if [ "${1:-}" = "" ]; then
    echo "Usage: bash server_scripts/run_final_rware_ippo_cpu.sh RUN_NAME [MAX_PARALLEL]" >&2
    exit 1
fi
...
dst = results_root / "_task_configs" / "robotic_warehouse_dynamic_final_ippo.json"
...
launch_job() {
    ...
    local run_id="final_robotic_warehouse_dynamic_ippo_${config_name}_s${seed}"
...
TASK_CONFIG_PATH="${RESULTS_ROOT}/_task_configs/robotic_warehouse_dynamic_final_ippo.json"
CONFIG_PATH="robotic_warehouse_dynamic/configs/ippo/best_01.json"
