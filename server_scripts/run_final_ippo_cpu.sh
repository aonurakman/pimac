...
if [ "${1:-}" = "" ]; then
    echo "Usage: bash server_scripts/run_final_ippo_cpu.sh RUN_NAME [MAX_PARALLEL]" >&2
    exit 1
fi
...
jobs = [
    ...
    (
        repo_root / "robotic_warehouse_dynamic" / "task.json",
        results_root / "_task_configs" / "robotic_warehouse_dynamic_final.json",
        {
            "episodes": 3000,
            "eval_every_episodes": 0,
            "test_rollouts": 100,
...
discover_config() {
    local task_dir="$1"
    find "${task_dir}/configs/ippo" -maxdepth 1 -type f -name '*.json' | sort
}
...
launch_job() {
    ...
    local run_id="final_${task_name}_ippo_${config_name}_s${seed}"
...
for task in "${TASKS[@]}"; do
    ...
    while IFS= read -r config_path; do
        configs+=("${config_path}")
    done < <(discover_config "${task}")
    if [ "${#configs[@]}" -eq 0 ]; then
        echo "No IPPO configs found under ${task}/configs/ippo" >&2
        exit 1
    fi
    ...
    for config_path in "${configs[@]}"; do
        for seed in ${SEEDS}; do
            if [ "${DRY_RUN}" = "1" ]; then
                launch_job "${task}" "${config_path}" "${task_config_path}" "${seed}"
                continue
            fi
