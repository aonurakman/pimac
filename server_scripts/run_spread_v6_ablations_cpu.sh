...
if [ "${1:-}" = "" ]; then
    echo "Usage: bash server_scripts/run_spread_v6_ablations_cpu.sh RUN_NAME [MAX_PARALLEL]" >&2
    exit 1
fi
...
repo_root = Path.cwd()
src = repo_root / "simple_spread_dynamic_hard" / "task.json"
dst = results_root / "_task_configs" / "spread_hard_final.json"
...
discover_configs() {
    find "simple_spread_dynamic_hard/configs/pimac_v6_ablation" -maxdepth 1 -type f -name '*.json' | sort
}
...
launch_job() {
    ...
    local run_id="final_simple_spread_dynamic_hard_pimac_v6_ablation_${config_name}_s${seed}"
    local log_path="${RESULTS_ROOT}/logs/${run_id}.log"

    local cmd=(
        venv/bin/python
        simple_spread_dynamic_hard/run.py
...
if [ "${#configs[@]}" -eq 0 ]; then
    echo "No configs found under simple_spread_dynamic_hard/configs/pimac_v6_ablation" >&2
    exit 1
fi
