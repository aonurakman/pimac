"""Run one benchmark experiment on the toy cooperative line world.

This task is intentionally tiny. The goal of this script is not abstraction; it is to make the
whole benchmark flow easy to inspect in one file.
"""
...
from toy_env.utils import (
    CoopLineWorldConfig,
    CoopLineWorldParallelEnv,
    EvalResult,
    build_summary,
    evaluate_rollouts,
...
def make_env(task_config: dict, seed: int, render_mode: str | None = None):
    """Build the toy parallel environment for one run or evaluation rollout."""
    env = CoopLineWorldParallelEnv(
        CoopLineWorldConfig(
            n_agents=int(task_config["n_agents"]),
...
def build_env_spec(task_config: dict, seed: int) -> ParallelEnvSpec:
    """Read the toy environment spaces once and convert them into the learner spec."""
...
def run_task(
    *,
    ...
    """Train one learner on the toy task and write the standard outputs."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Run one experiment on the toy cooperative line world.")
