"""
PIMACV6 implementation for parallel multi-agent environments.

PIMACV6 extends `pimac_v1` with context-reliance-gated FiLM policy conditioning:
...
class PIMACV6ActorRNN(nn.Module):
    """
    ...
    def __init__(
        ...
        super().__init__()
        assert len(widths) == (int(num_hidden) + 1), "PIMACV6 actor widths and layer count mismatch."
...
class PIMACV6(ParallelLearner):
    """
    Shared PIMACV6 implementation core backing the public parallel learner.
    ...
    def normalize_config(config: dict) -> dict:
        return normalize_config(config, PIMACV6_DEFAULT_CONFIG)
    ...
    def __init__(self, env_spec: ParallelEnvSpec, config: dict, device: str = "cpu"):
        ...
        self.actor_net = PIMACV6ActorRNN(
