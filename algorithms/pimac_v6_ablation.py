"""
PIMACV6 ablation implementation for parallel multi-agent environments.

PIMACV6Ablation extends `pimac_v1` with context-reliance-gated FiLM policy conditioning:
...
- The decentralized actor predicts student context (`ctx_mu`) from local
  recurrent features, then modulates policy features with FiLM.
- A scalar context-reliance signal gates FiLM strength, but is trained only
  through the policy path.
- Student context is trained by direct distillation against teacher context,
  while PPO remains on-policy.

A config-controlled `ablation` mode can override one mechanism at a time:
- `always_on_gate`
- `always_off_gate`
- `no_distill_weight`
...
__all__ = [
    "PIMACV6AblationActorRNN",
    "SetTokenTeacher",
    "SetValueCritic",
    "PIMACV6Ablation",
    "PIMACV6_ABLATION_DEFAULT_CONFIG",
]
...
PIMACV6_ABLATION_DEFAULT_CONFIG = {
    ...
    "ctx_reliance_min": -6.0,
    "ctx_reliance_max": 4.0,
...
class PIMACV6AblationActorRNN(nn.Module):
    """
    ...
    - scalar context reliance (`ctx_reliance`) for policy gating.

    Policy conditioning follows reliance-gated FiLM:
    - FiLM scale/shift are predicted from student context mean.
    - A scalar gate from student context reliance scales FiLM strength.
    ...
    def __init__(
        ...
        rnn_hidden_dim: int,
        ctx_dim: int,
        ctx_reliance_min: float = -6.0,
        ctx_reliance_max: float = 4.0,
        ablation: Optional[str] = None,
    ):
        super().__init__()
        assert len(widths) == (int(num_hidden) + 1), "PIMACV6 ablation actor widths and layer count mismatch."
        ...
        self.ctx_reliance_head = nn.Linear(int(rnn_hidden_dim), 1)
        ...
        self.ctx_reliance_min = float(ctx_reliance_min)
        self.ctx_reliance_max = float(ctx_reliance_max)
    ...
    def forward(
        ...
        if self.ablation in {"always_on_gate", "always_off_gate"}:
            context_reliance = torch.zeros(
                batch_size,
                num_timesteps,
                1,
            ...
            context_reliance = torch.clamp(
                self.ctx_reliance_head(recurrent_features),
                min=self.ctx_reliance_min,
                max=self.ctx_reliance_max,
            )
        ...
        # Higher predicted reliance strengthens context-conditioned modulation.
        if self.ablation == "always_on_gate":
            modulation_gate = torch.ones_like(context_reliance)
        elif self.ablation == "always_off_gate":
            modulation_gate = torch.zeros_like(context_reliance)
        else:
            modulation_gate = torch.sigmoid(self.gate_weight * context_reliance + self.gate_bias)

        # Context-reliance-gated FiLM modulation over recurrent policy features.
...
class SetValueCritic(nn.Module):
    ...
    predicted from concatenated coordination tokens via `rho`.
    """
    ...
    def __init__(
        ...
        self.value_mlp = _build_mlp(in_dim=value_input_dim, hidden_sizes=tuple(critic_hidden_sizes), out_dim=1)
...
class PIMACV6Ablation(ParallelLearner):
    """
    Shared PIMACV6 ablation implementation core backing the public parallel learner.
    ...
    def normalize_config(config: dict) -> dict:
        normalized = normalize_config(config, PIMACV6_ABLATION_DEFAULT_CONFIG)
        ablation = normalized.get("ablation")
        if ablation not in {None, "always_on_gate", "always_off_gate", "no_distill_weight"}:
            raise ValueError(
                "Unsupported pimac_v6_ablation mode: "
                f"{ablation!r}. Expected one of None, 'always_on_gate', 'always_off_gate', 'no_distill_weight'."
            )
        return normalized
    ...
    def __init__(self, env_spec: ParallelEnvSpec, config: dict, device: str = "cpu"):
        ...
        if self.ablation == "no_distill_weight":
            self.distill_weight = 0.0
            self.config["distill_weight"] = 0.0
        self.teacher_ema_tau = float(max(0.0, min(1.0, config["teacher_ema_tau"])))

        self.actor_net = PIMACV6AblationActorRNN(
            ...
            ctx_reliance_min=float(config["ctx_reliance_min"]),
            ctx_reliance_max=float(config["ctx_reliance_max"]),
    ...
    def _actor_forward(self, obs: torch.Tensor, return_aux: bool = False):
        ...
        if return_aux:
            ...
            aux = {
                ...
                "ctx_reliance": flattened_aux["ctx_reliance"].reshape(batch_size, num_agents, num_timesteps, -1).permute(0, 2, 1, 3),
    ...
    def _compute_update_diagnostics(
        ...
        with torch.no_grad():
            ...
            cosine_alignment = F.cosine_similarity(student_context, teacher_context, dim=-1)
            selected_cosine_alignment = cosine_alignment[combined_mask.bool()]
            if selected_cosine_alignment.numel() > 0:
                teacher_student_cosine_mean = selected_cosine_alignment.mean()
                teacher_student_cosine_std = selected_cosine_alignment.std(unbiased=False)
            else:
                teacher_student_cosine_mean = torch.tensor(0.0, device=self.device)
                teacher_student_cosine_std = torch.tensor(0.0, device=self.device)

            selected_reliance_values = ctx_reliance.squeeze(-1)[combined_mask.bool()]
            if selected_reliance_values.numel() > 0:
                ctx_reliance_mean = selected_reliance_values.mean()
                ctx_reliance_std = selected_reliance_values.std(unbiased=False)
            else:
                ctx_reliance_mean = torch.tensor(0.0, device=self.device)
                ctx_reliance_std = torch.tensor(0.0, device=self.device)
        ...
        return {
            ...
            "teacher_student_cosine_mean": teacher_student_cosine_mean,
            "teacher_student_cosine_std": teacher_student_cosine_std,
            "ctx_reliance_mean": ctx_reliance_mean,
            "ctx_reliance_std": ctx_reliance_std,
            "gate_mean": gate_mean,
            "gate_std": gate_std,
        }
    ...
    def _run_update(self, global_step: int, episode_index: int) -> Optional[UpdateReport]:
        ...
        mean_teacher_student_cosines: list[float] = []
        mean_teacher_student_cosine_stds: list[float] = []
        mean_ctx_reliance_means: list[float] = []
        mean_ctx_reliance_stds: list[float] = []
        ...
        for _ in range(self.num_epochs):
            ...
            student_context_reliance = actor_aux["ctx_reliance"]
            context_gate = actor_aux["gate"].squeeze(-1)
            ...
            diagnostics = self._compute_update_diagnostics(
                ...
                ctx_reliance=student_context_reliance,
                gate=context_gate,
                student_context=student_context_mean,
                tokens=tokens,
            ...
            mean_teacher_student_cosines.append(float(diagnostics["teacher_student_cosine_mean"].detach().item()))
            mean_teacher_student_cosine_stds.append(float(diagnostics["teacher_student_cosine_std"].detach().item()))
            mean_ctx_reliance_means.append(float(diagnostics["ctx_reliance_mean"].detach().item()))
            mean_ctx_reliance_stds.append(float(diagnostics["ctx_reliance_std"].detach().item()))
        ...
        report = UpdateReport(
            ...
            extras={
                ...
                "teacher_student_cosine_mean": float(np.mean(mean_teacher_student_cosines)),
                "teacher_student_cosine_std": float(np.mean(mean_teacher_student_cosine_stds)),
                "ctx_reliance_mean": float(np.mean(mean_ctx_reliance_means)),
                "ctx_reliance_std": float(np.mean(mean_ctx_reliance_stds)),
    ...
    def act(
        ...
        if agent_index is None:
            raise ValueError("PIMACV6.act requires agent_index. Prefer act_parallel for parallel environments.")
