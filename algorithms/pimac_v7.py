"""
PIMACV7 implementation for parallel multi-agent environments.

PIMACV7 extends `pimac_v1` with a context-conditioned policy head:
...
- A scalar context-reliance signal gates the residual strength and is trained
  only through the policy path.
...
class PIMACV7ActorRNN(nn.Module):
    """
    ...
    Policy conditioning follows reliance-gated head adaptation:
    - A scalar gate from student context reliance scales the adaptation strength.
    ...
    def __init__(
        ...
        super().__init__()
        assert len(widths) == (int(num_hidden) + 1), "PIMACV7 actor widths and layer count mismatch."
        if int(hypernet_rank) <= 0:
            raise ValueError("PIMACV7 hypernet_rank must be a positive integer.")
        ...
        self.ctx_reliance_head = nn.Linear(self.hidden_dim, 1)
    ...
    def forward(
        ...
        # Higher predicted reliance strengthens context-conditioned adaptation.
...
class PIMACV7(ParallelLearner):
    """
    Shared PIMACV7 implementation core backing the public parallel learner.
    ...
    The actor uses head-only context adaptation, with the residual strength
    controlled by a learned scalar reliance signal.
    """

    @staticmethod
    def normalize_config(config: dict) -> dict:
        return normalize_config(config, PIMACV7_DEFAULT_CONFIG)
    ...
    def __init__(self, env_spec: ParallelEnvSpec, config: dict, device: str = "cpu"):
        ...
        self.actor_net = PIMACV7ActorRNN(
    ...
    def act(
        ...
        if agent_index is None:
            raise ValueError("PIMACV7.act requires agent_index. Prefer act_parallel for parallel environments.")
