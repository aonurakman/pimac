"""
PIMACV2 implementation for parallel multi-agent environments.

PIMACV2 extends `pimac_v1` with uncertainty-gated FiLM policy conditioning:
...
- A scalar uncertainty gate derived from `logvar` controls FiLM strength.
...
class PIMACV2ActorRNN(nn.Module):
    """
    ...
    def __init__(
        ...
        super().__init__()
        assert len(widths) == (int(num_hidden) + 1), "PIMACV2 actor widths and layer count mismatch."
...
class PIMACV2(ParallelLearner):
    """
    Shared PIMACV2 implementation core backing the public parallel learner.
    ...
    def normalize_config(config: dict) -> dict:
        return normalize_config(config, PIMACV2_DEFAULT_CONFIG)
    ...
    def __init__(self, env_spec: ParallelEnvSpec, config: dict, device: str = "cpu"):
        ...
        self.actor_net = PIMACV2ActorRNN(
    ...
    def act(
        ...
        if agent_index is None:
            raise ValueError("PIMACV2.act requires agent_index. Prefer act_parallel for parallel environments.")
