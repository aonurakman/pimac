"""
MAPPO (Multi-Agent PPO) implementation for parallel multi-agent environments.

This module implements a canonical CTDE MAPPO variant with:
- decentralized execution through one shared recurrent actor policy,
- centralized training through a critic over masked joint observations, and
- team-level GAE(lambda) with clipped PPO policy optimization.

Execution-time action selection uses only local observations. Centralized
features are used strictly inside value learning.
"""
...
class MAPPOActorRNN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert len(widths) == (int(num_hidden) + 1), "MAPPO actor widths and layer count mismatch."
...
class CentralizedValueCritic(nn.Module):
    """
    Centralized team-value critic.

    The critic consumes:
    - masked joint observations (inactive slots zeroed), and
    - active-mask channels.

    Inputs are flattened over the configured agent axis to keep a fixed input width.
    """
    ...
    def __init__(
        ...
        critic_input_dim = self.num_agents * self.obs_dim + self.num_agents
        self.value_mlp = _build_mlp(critic_input_dim, tuple(hidden_sizes), out_dim=1)
    ...
    def forward(self, obs: torch.Tensor, active_mask: torch.Tensor) -> torch.Tensor:
        ...
        if num_agents > self.num_agents:
            raise ValueError(
                f"Critic configured for {self.num_agents} agents but received {num_agents}."
            )

        if num_agents < self.num_agents:
            # Episodes with smaller active rosters are right-padded to critic width.
            pad_agents = self.num_agents - num_agents
            obs = torch.cat(
                [
                    obs,
                    torch.zeros(
        ...
        masked_obs = obs * active_mask.unsqueeze(-1)

        flat_obs = masked_obs.reshape(batch_size, num_timesteps, self.num_agents * self.obs_dim)
        flat_mask = active_mask.reshape(batch_size, num_timesteps, self.num_agents)
        critic_input = torch.cat([flat_obs, flat_mask], dim=-1)

        values = self.value_mlp(critic_input.reshape(batch_size * num_timesteps, -1)).reshape(
            batch_size,
            num_timesteps,
        )
        return values
...
class MAPPO(ParallelLearner):
    """
    Shared MAPPO implementation core backing the public parallel learner.
    ...
    def _build_minibatch_tensors(self, batch: list[dict]) -> dict[str, torch.Tensor]:
        ...
        The agent axis is always padded to `self.num_agents` so the centralized
        critic sees one stable input dimensionality.
        """
        max_num_timesteps = max(int(episode["T"]) for episode in batch)
        batch_max_agents = max(int(episode["N"]) for episode in batch)
        if batch_max_agents > self.num_agents:
            raise ValueError(
                f"Found episode with {batch_max_agents} agents, but MAPPO was configured for {self.num_agents}."
            )
