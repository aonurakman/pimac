"""VDN with the shared parallel benchmark API."""
...
__all__ = ["AgentRNN", "VDN", "VDN_DEFAULT_CONFIG"]
...
class VDN(ParallelLearner):
    ...
    def normalize_config(config: dict) -> dict:
        return normalize_config(config, VDN_DEFAULT_CONFIG)
    ...
    def __init__(self, env_spec: ParallelEnvSpec, config: dict, device: str = "cpu"):
        ...
        self.normalize_by_active = bool(config["normalize_by_active"])
        ...
        self.optimizer = optim.Adam(list(self._agent_parameters()), lr=float(config["lr"]))
    ...
    def _mix_q_tot(self, chosen_q: torch.Tensor, active_mask: torch.Tensor) -> torch.Tensor:
        q_sum = (chosen_q * active_mask).sum(dim=2)
    ...
    def _run_update(self, global_step: int) -> UpdateReport:
        ...
        for _ in range(self.num_epochs):
            ...
            loss.backward()
            grad_norm = _parameter_grad_norm(list(self._agent_parameters()))
