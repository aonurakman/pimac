"""Run one benchmark experiment on dynamic-team `simple_spread_v3`.

This runner keeps the full experiment loop visible: curriculum sampling, training, validation,
checkpoint selection, test-time evaluation, and report writing.
"""
...
def make_env(task_config: dict, seed: int, n_agents: int, render_mode: str | None = None):
    """Build one dynamic-team MPE environment instance."""
    try:
    ...
    *,
    ...
    try:
        for episode_index in range(int(task_config["episodes"])):
            # The curriculum decides how many agents train together in this episode.
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Run one experiment on dynamic simple_spread_v3.")
